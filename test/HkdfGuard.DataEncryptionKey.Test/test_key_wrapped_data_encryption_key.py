...
def test_encrypt_decrypt_round_trips() -> None:
    data_encryption_key, _ = _create_key()
    ...
    encrypted = data_encryption_key.encrypt(plaintext)
    assert len(encrypted) == len(expected) + 12 + 16

    decrypted = bytearray(len(expected))
    decrypted_length = data_encryption_key.decrypt(encrypted, decrypted)
...
def test_encrypt_decrypt_with_aad_round_trips() -> None:
    ...
    encrypted = data_encryption_key.encrypt(plaintext, aad)

    decrypted = bytearray(len(expected))
    decrypted_length = data_encryption_key.decrypt(encrypted, decrypted, aad)
...
def test_decrypt_with_mismatched_aad_raises() -> None:
    data_encryption_key, _ = _create_key()
    plaintext = bytearray(b"top secret")
    encrypted = data_encryption_key.encrypt(plaintext, b"context-a")

    result = bytearray(len(plaintext))
    with pytest.raises(InvalidTag):
        data_encryption_key.decrypt(encrypted, result, b"context-b")
...
def test_encrypt_and_decrypt_reuse_the_cached_session_across_calls() -> None:
    ...
    data_encryption_key, wrapper = _create_key()
    encrypted1 = data_encryption_key.encrypt(bytearray(b"one"))
    encrypted2 = data_encryption_key.encrypt(bytearray(b"two"))

    data_encryption_key.decrypt(encrypted1, bytearray(3))
    data_encryption_key.decrypt(encrypted2, bytearray(3))
